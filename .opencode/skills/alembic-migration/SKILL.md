...
1. **Always review autogenerated migrations** - They may miss custom operations
2. **Test both upgrade and downgrade** - Ensure rollback works
3. **Use nullable=True for new columns** on existing tables (or provide default)
4. **Create indexes concurrently** for large tables in production
5. **Data migrations** - Separate from schema migrations when possible
